    ...
    tabItems(
      ...
      tabItem(tabName = "Tables_tab",
              fluidPage(
                ...
                fluidRow(
                  column(10,DT::dataTableOutput("CFA_Table_DF"))
                )
              )
      ),
      ...
      tabItem(tabName = "AVE",
              fluidPage(
                ...
                fluidRow(
                  column(4,uiOutput("ui_ave_title"), DT::dataTableOutput("ave_table_DF"))
                ),
                fluidRow(
                  column(4, textOutput("alpha_text"))
                ),
                fluidRow(
                  column(4,(HTML("<br/>")), (HTML("<br/>")), uiOutput("ui_ICC_title"), DT::dataTableOutput("icc_table_DF"))
                )
              ))
...
server <-
  function(input, output, session) {
    ...
    extract_factor_names <- function(var_names){
      ...
      var_names_char <- purrr::map_chr(var_names_char, function(x){
    ...
    extract_set_of_items <- function(var_names){
      ...
      items_and_names <- purrr::map(factor_names, function(x){
    ...
    proposed_cfa_model <- function(factor_names, set_of_items){
      ...
      single_indicator <- purrr::map_lgl(set_of_items, function(x) !stringr::str_detect(x, "\\+"))
        ...
        for (z in 1:length(set_of_items)){
          for( i in 1:(length(set_of_items))){
            if (z >= i){ next
              ...
              ###
              single_indicator <- purrr::map_lgl(set_of_items_other, function(x) !stringr::str_detect(x, "\\+"))
    ...
    one_cfa_models <- function(factor_names, set_of_items){
      ...
      uncorrelated_to_latent <- purrr::map_chr(factor_names, function(x) paste("LatentCMV ~~ 0*", x, sep = ""))
      ...
      ##### uncorrelated to other constructs
      uncorrelated_to_latent_fixed <- purrr::map_chr(factor_names, function(x) paste("LatentCMV_fixed ~~ 0*", x, sep = ""))
    ...
    run_cfa <- function(data,
      ...
      host <- waiter::Hostess$new("loader")

      w <- waiter::Waiter$new(html = tagList(span("Running CFA models..."), host$get_loader(stroke_color = "#ffffff",
                                                                                    text_color = "white",
      ...
      for (i in seq_along(All_CFA_Models)){
        Fitted_Models[[i]] <- lavaan::cfa(All_CFA_Models[[i]], data=data, mimic = mimic, estimator = estimator, orthogonal = orthogonal, cluster = cluster, meanstructure=TRUE, check.lv.names = FALSE)
 ...
 output_in_data_frame <- function(fitted_models){
      output_fits <- purrr::map(fitted_models, extract_fit)
    ...
    get_cfa_data_frame <- reactive({
      ...
      Comparisons2 <- purrr::map_df(to_compare_models[2:length(to_compare_models)], function(x){
        model_name <- names(x)
        model1 <- to_compare_models[[1]]
        comp <- lavaan::anova(model1,x)
    ...
    output$manual_items <- renderUI({
      ...
      for (i in seq_len(input$how_many_factors) ){
        list_of_factors <- paste(list_of_factors,'shinyWidgets::pickerInput(
    ...
    CorrelationTableDT <- reactive({
      ...
      if (is.null(Selected_Items$items)){
        ...
        mean_data <- purrr::map_df(df.subset, function(x) {
          x1 <- mean(x, na.rm = TRUE)
          return(x1)
        })
        sd_data <- purrr::map_df(df.subset, function(x) {
          x1 <- sd(x, na.rm = TRUE)
          return(x1)
        })
    ...
    output$downloadCor_Full <- downloadHandler(
      ...
      content = function(file) {
        openxlsx::write.xlsx(CorrelationTableDT_Full(), file, asTable = FALSE,
                   rowNames = FALSE,
                   colNames = TRUE,
                   firstActiveRow  = 2,
                   firstActiveCol = 3,
                   headerStyle = openxlsx::createStyle(textDecoration = "ITALIC")
        )
      }
    )
    ...
    output$downloadCor_LowerT <- downloadHandler(
      ...
      content = function(file) {
        openxlsx::write.xlsx(CorTab_LowerTriang(), file, asTable = FALSE,
                   rowNames = FALSE,
                   colNames = TRUE,
                   firstActiveRow  = 2,
                   firstActiveCol = 3,
    ...
    output$data_summary <- renderPrint({
      ...
      if (is.null(Selected_Items$items)){
        ...
        df.subset <- df[, Selected_Items$items]
        out <- print(summarytools::dfSummary(df.subset, round.digits = 3, graph.magnif = 1.2, valid.col = FALSE), omit.headings = FALSE, method = 'render')
    ...
    edited_cfa_table <- reactive({
      ...
      x <- get_cfa_data_frame()
      fit_indices <- intersect(dplyr::all_of(fit_indices) , names(x))
      x <- dplyr::select(x, all_of(fit_indices))
      return(x)

    })

    output$CFA_Table_DF <- DT::renderDataTable({
      req(input$cfa_columns)
      x <- edited_cfa_table()
      return(x)
    ...
    output$CFA_Table_download <- downloadHandler(
      ...
      content = function(file) {
        openxlsx::write.xlsx(edited_cfa_table(), file, asTable = FALSE,
                   rowNames = TRUE,
                   colNames = TRUE,
                   headerStyle = openxlsx::createStyle(textDecoration = "ITALIC")
    ...
    extract_reliabilities <-   function(cfa_model){
      ...
      if(is.null(nrow(relia_x))){
        ...
        cors_x <- round(as.data.frame(lavaan::lavInspect(cfa_model,"cor.lv")^2),3)
    ...
    ave_table <- reactive({
      x <- get_run_cfa()
      ave_output <- purrr::map(x, function(x) {
    ...
    output$ave_table_DF <- DT::renderDataTable({
    ...
    text_reversed <- reactive({
      x <- get_run_cfa()
      reverse_output <- purrr::map(x, function(x) {
    ...
    output$AVE_Table_download <- downloadHandler(
      ...
      content = function(file) {
        openxlsx::write.xlsx(ave_table(), file, asTable = FALSE,
                   rowNames = TRUE,
                   colNames = TRUE,
                   headerStyle = openxlsx::createStyle(textDecoration = "ITALIC")
    ...
    extract_icc <- function(cfa_model){
      factor_Scores <- lavaan::lavPredict(cfa_model, method = "ML")
      ...
      if(t1 > 1){
        y <- purrr::map_df(icc_data_Frame[, 1:t1], function(x){
    ...
    icc_table <- reactive({
      ...
      if(input$isThereCluster == "Yes"){
        ...
        icc_output <- purrr::map(x, function(x) {
    ...
    output$icc_table_DF <- DT::renderDataTable({
      req(input$ave_lavaan_model_to_show)
      req(input$isThereCluster)
      if(input$isThereCluster == "Yes"){
        x <- icc_table()
    ...
    output$ICC_Table_download <- downloadHandler(
      ...
      content = function(file) {
        openxlsx::write.xlsx(icc_table(), file, asTable = FALSE,
                   rowNames = TRUE,
                   colNames = TRUE,
                   headerStyle = openxlsx::createStyle(textDecoration = "ITALIC")
